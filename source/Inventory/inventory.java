...
public class inventory {
    ...
    public void switchPlants(int index1, int index2, int category){
        ...
        if (category == 1){
            //Empty slot
            if (inventoryList[index1] == null || inventoryList[index2] == null || index1>6 || index2>6){
                return;
            }
            //Tuker
            Plants temp = inventoryList[index1];
            inventoryList[index1] = inventoryList[index2];
            inventoryList[index2] = temp;
        }
        //Tuker gameDeck
        else{
            //Empty slot
